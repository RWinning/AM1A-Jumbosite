<?php
	...
	class LoginClass
	{
		...
		{
			global $database;
			$query = "SELECT * FROM `login2` WHERE `email` = '".$emailaddress."'";
		...
		{
			global $database;
			$query = "SELECT * FROM `login2` WHERE `email` = '".$email."' AND `password` = '".$password."'";
			//echo $query; exit();
			$result = $database ->fire_query($query);
			if (mysql_num_rows($result) > 0 )
			{
	   ...
	   
	   {
		  ...
		  $query = "SELECT * FROM `login2` WHERE `email` = '".$email."'";
	   ...
	   public static function insert_into_loginClass($post_array)
	   {
			...
			$query = "INSERT INTO  `login2` (`id`,`email`,`password`,`userrole`,`activated`,`activationdate`)
	   ...
	   {
	   			$query = "SELECT * FROM `login2` WHERE `id` = '".$_SESSION['id']."'";
